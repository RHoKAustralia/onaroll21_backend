...
@if($errors->any())
    ...
    @endif
    <h2>{{ t('master.updateuser') }}</h2>
    {{ Form::open(array('url' => 'user/update','files'=>true)) }}
        {{ Form::hidden('id',$user->id)}}
        {{ FormField::username(['label'=>trans('master.username'),'value'=>$user->username]) }}
        {{ FormField::password(['label'=>trans('master.password')]) }}
        {{ FormField::email(['label'=>trans('master.email'),'value'=>$user->email]) }}
        ...
        <br /><br />
        {{ Form::submit(t('master.updateuser'),array('class'=>'btn btn-success','name'=>'update'))}}
    {{ Form::close() }}
@stop